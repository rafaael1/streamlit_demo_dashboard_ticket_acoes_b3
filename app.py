...
Empresas = ['ITSA4', 'ENBR3', 'EZTC3', 'ITUB4', 'STBP3', 'ENAT3', 'ALUP4', 'BBAS3', 'TRPL4', 'SAPR11',
...
try:
  ...
  if dados.empty:
    st.error("Não foi possivel obter dados para " + Selecao + ".SA. Por favor, tente outra ação.")
    raise Exception
except:
  raise Exception
else:
  ...
  if st.checkbox('Mostrar dados em tabela'):
    ...
    styler = dados.reset_index().style.hide().format(subset=['Open','High','Low','Close','Adj Close'], decimal=',', precision=2).format(subset=['Volume'], formatter='{:,}', thousands='.')
